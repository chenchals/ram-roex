! roexpr.for   symmetric filter, parameters p & r
...
!     the maximum allowed value of pl or pl (pmax)
...
!  4. Start values for the parameters & maximum shift: pl,pl,r,max_shift
...
if(ssq(j)<sscheck - 0.1.and.ssq(j)>0.5d0) then
    ...
    erb = erb + (1. - r_lin) / pl * (2. - (2. + up_erb_lim * pl)&
            * exp(-up_erb_lim * pl)) + up_erb_lim * r_lin
    ! commented out	  erb = 2./pl+2./pl

    write(8, 9015)No, cf
    9015    format(' Noise spectrum level =', f5.1, '  cf =', f8.1, ' Hz')
    write(8, 9020)pl, r, xk, erb, erb * cf
    9020    format(' p =', f5.1, ' r = ', f6.1, ' k(dB) =', f5.1, ' erb =', f6.3, &
            '( =', f6.1, ' Hz)', /)
...
subroutine input(title, corr_file, No)
    ...
    if(corr_file=='none') then
        numeric = .false.
        write(8, 9060)
        9060    format(' roexpr : ( analytical integration)')
        write(8, 9070)
        9070    format(' No threshold correction used')
    else
        numeric = .true.
        write(8, 9080)
        9080    format(' roexpr : ( numerical integration)')
...
real(kind=8) function sscalc(par)
    ...
    if(pl>pmax)   sscalc = sscalc + 1000.0d0
    if(pl<0.1d0) sscalc = sscalc + 1000.0d0
    if(n_print>0) then
        if(mod(sscount, n_print)==0.or.sscount==1)&
                write(6, 9000)sscount, pl, r, sscalc
    endif
    9000    format(1x, i4, ' p =', f7.3, '  r = ', f9.2, ' ssq =', f9.3)
end
!--------------------------------------------
...
real(kind=8) function p_const(ns_shift)
    ...
    ! frequency pl or pl to get the shifted frequency pl or pl.
