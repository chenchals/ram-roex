...
real(kind = 8) function brent(ax, bx, cx, f, tol, xmin)
    ...
    write(*,*)'brent called with: ax=',ax,'bx=',bx,' cx=',cx,' tol=',tol,' xmin=',xmin
    a = min(ax, cx)
    b = max(ax, cx)
    v = bx
    w = v
    ...
    do iter = 1, itmax
        ...
        if(abs(e)>tol1) then
            ...
            if(u - a<tol2.or.b - u<tol2) then
                d = sign(tol1, xm - x)
                write(*,*)' goto2@277...,iter=',iter
                goto 2
            endif

        endif
        write(*,*)'fall through...to label goto 1 iter=',iter
        write(*,*)'a=',a,'b=',b,' e=',e,' d=',d,' x=',x
        1      if(x>=xm) then
            e = a - x
        else
            e = b - x
